...
/// Serializes a given serializable value into Universal Binary JSON format and writes it to the provided writer.
pub fn to_writer<W, T>(writer: &mut W, value: &T) -> Result<(), UbjError>
where
    W: std::io::Write,
    T: serde::Serialize,
{
    let mut serializer = UbjSerializer { writer };
    value.serialize(&mut serializer)?;
    writer.flush().map_err(UbjError::IO)?;
    Ok(())
}

/// Serializes a given serializable value into Universal Binary JSON format and returns the resulting bytes into an in-memory buffer.
    ...
    W: std::io::Write,
{
    ...
    fn serialize_char(self, c: char) -> Result<Self::Ok, Self::Error> {
        ...
        if utf8_code > 127 {
            return Err(UbjError::OutOfRange(c));
        }
...
// TODO struct MapKeySerializer<'a, W> where W: std::io::Write { writer: &'a mut W }
// TODO impl<'a, W> serde::Serializer for &mut MapKeySerializer<'a, W> where W: std::io::Write { ... }
