...
pub struct UbjSerializer<'a, W> where W: IoWrite
{
    writer: &'a mut W,
}


impl<'a, W> UbjSerializer<'a, W> where W: IoWrite
{
    ...
    fn write_marker_and_payload(&mut self, marker: UbjMarker, payload: &[u8]) -> Result<(), UbjError>
    {
    ...
    fn write_i8(&mut self, number: i8) -> Result<(), UbjError>
    {
        self.write_marker_and_payload(UbjMarker::Int8, &number.to_be_bytes())
    }

    fn write_u8(&mut self, number: u8) -> Result<(), UbjError>
    {
        ...
        //
        self.write_marker_and_payload(UbjMarker::Uint8, &number.to_be_bytes())
    }

    fn write_i16(&mut self, number: i16) -> Result<(), UbjError>
    {
        if number >= i8::MIN as i16 && number < 0
        {
            return self.write_i8(number as i8);
        }
        else if number >= 0 && number <= u8::MAX as i16
        {
            return self.write_u8(number as u8);
        }
        self.write_marker_and_payload(UbjMarker::Int16, &number.to_be_bytes())
    }

    fn write_i32(&mut self, number: i32) -> Result<(), UbjError>
    {
        if number >= i16::MIN as i32 && number <= i16::MAX as i32
        {
            return self.write_i16(number as i16);
        }
        self.write_marker_and_payload(UbjMarker::Int32, &number.to_be_bytes())
    }

    fn write_i64(&mut self, number: i64) -> Result<(), UbjError>
    {
        if number >= i32::MIN as i64 && number <= i32::MAX as i64
        {
            return self.write_i32(number as i32);
        }
        self.write_marker_and_payload(UbjMarker::Int64, &number.to_be_bytes())
    }

    // TODO fn write_huge(&mut self, number: u64) -> Result<(), UbjError> {

    fn write_size_and_payload(&mut self, v: &str) -> Result<(), UbjError>
    {
        ...
        if len <= (i64::MAX as usize) {
            ...
            return Err(UbjError::UnsupportedType("alloc::string::String with len > i64::MAX"));
        }
...
impl<'a, W> serde::Serializer for &mut UbjSerializer<'a, W> where W: IoWrite
{
    type Ok = ();
    type Error = UbjError;
    ...
    fn serialize_u64(self, v: u64) -> Result<Self::Ok, Self::Error>
    {
        ...
        Err(UbjError::UnsupportedType("u64"))
    }
    ...
    fn serialize_char(self, c: char) -> Result<Self::Ok, Self::Error>
    {
        ...
        if utf8_code > 127 {
            return Err(UbjError::CharOutOfRange(c));
        }
    ...
    fn serialize_some<T>(self, value: &T) -> Result<Self::Ok, Self::Error> where T: ?Sized + serde::Serialize,
    {
    ...
    {
        self.write_marker(UbjMarker::OpeningBrace)?;
        self.write_size_and_payload(variant)?;
    ...
    {
        Err(UbjError::UnsupportedType("std::collections::HashMap"))
    }
...
impl<'a, W> serde::ser::SerializeSeq for &mut UbjSerializer<'a, W> where W: IoWrite
{
    type Ok = ();
    type Error = UbjError;
...
impl<'a, W> serde::ser::SerializeTuple for &mut UbjSerializer<'a, W> where W: IoWrite
{
    type Ok = ();
    type Error = UbjError;
...
impl<'a, W> serde::ser::SerializeTupleStruct for &mut UbjSerializer<'a, W> where W: IoWrite
{
    type Ok = ();
    type Error = UbjError;
...
impl<'a, W> serde::ser::SerializeTupleVariant for &mut UbjSerializer<'a, W> where W: IoWrite
{
    type Ok = ();
    type Error = UbjError;
...
impl<'a, W> serde::ser::SerializeStructVariant for &mut UbjSerializer<'a, W> where W: IoWrite,
{
    type Ok = ();
    type Error = UbjError;
    ...
    where T: ?Sized + serde::Serialize,
    {
        self.write_size_and_payload(key)?;
        value.serialize(&mut (**self))?;
        Ok(())
    }
...
impl<'a, W> serde::ser::SerializeStruct for &mut UbjSerializer<'a, W> where W: IoWrite,
{
    type Ok = ();
    type Error = UbjError;
...
impl<'a, W> serde::ser::SerializeMap for &mut UbjSerializer<'a, W> where W: IoWrite,
{
    type Ok = ();
    type Error = UbjError;
    ...
    where T: ?Sized + serde::Serialize,
    {
        ...
        // TODO key.serialize(MapKeySerializer { writer: self.writer })?;
        Ok(())
    }
...
// TODO struct MapKeySerializer<'a, W> where W: IoWrite { writer: &'a mut W }
// TODO impl<'a, W> serde::Serializer for &mut MapKeySerializer<'a, W> where W: IoWrite { ... }
// TODO use serde::ser::Impossible;
...
where W: IoWrite, T: serde::Serialize
{
